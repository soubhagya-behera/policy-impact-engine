...
- The feed supports read/unread state with an explicit "mark read" action; it is a record, not a message queue — no external broker is involved. **The REST feed endpoints are IMPLEMENTED (Phase 10B-2B, with Phase 8B authentication):** `GET /api/v1/me/notifications` (newest first), `GET /api/v1/me/notifications/unread` (unread only, newest first), and `POST /api/v1/me/notifications/{notificationId}/read` (idempotent mark-read returning the same resource/`readAt` on repeat). Identity comes only from `AuthenticatedUsers.requireUserId(authentication)`; the `notificationId` path variable is only the resource identifier. Cross-user access behaves as not-found (404, never 403); malformed UUIDs map to the existing 400 behavior; missing/invalid tokens map to the existing 401 behavior. DTO mapping runs inside the existing service transactions (no `EntityGraph`/fetch join in this slice; the per-row lazy-load cost is accepted for this bounded feed and pagination/index hardening belongs to Phase 13). No migration (V1–V16 unchanged).
- Emission is part of the single pipeline completion path; both manual and scheduled checks produce notifications identically. **Automatic observation fan-out to the single policy owner is IMPLEMENTED (Phase 10B-1, service layer only); the authenticated REST feed is IMPLEMENTED (Phase 10B-2B).** A notification carries the assessment reference, and the assessment/recommendation details behind it are readable through the authenticated assessment/recommendation REST surface (IMPLEMENTED — see §28), closing the read-side loop from notification to explanation without touching emission.
...
- **Foundation IMPLEMENTED (Phase 11A):** Flyway V17 `audit_event` table with the frozen six-event catalog CHECK, hash length CHECKs, `UNIQUE(prev_hash)` / `UNIQUE(event_hash)` chain guards, a partial single-genesis index, and the actor feed index; immutable `AuditEvent` entity; pure Spring-free `AuditChain` canonicalization + SHA-256 (golden vector pinned); `AuditService.append` in dedicated short transactions with `UNIQUE`-retry convergence and a bounded budget (audit failure never rolls back business state); service-level user-scoped reads. **Verification IMPLEMENTED (Phase 11B):** read-only `AuditVerificationService` over a scalar verification projection (actor id without loading user entities), linkage-ordered walk from the single genesis recomputing every `event_hash` through the frozen `AuditChain` with an immutable first-failure result; empty chain is valid with count zero. Verification detects inconsistency on recomputation but does not prove historical completeness against tail truncation (ADR-023 §9). **Emission IMPLEMENTED (Phase 11C):** the six frozen event types are wired post-commit as a best-effort witness (auth register/login and policy registration from controllers; owner assignment and preference upsert/delete through service facades with per-change granularity); audit failure propagates without rolling back committed business state; no-op and failed operations stay silent. **Feed IMPLEMENTED (Phase 11D):** authenticated `GET /api/v1/me/audit-events` (principal-only identity, repository-level actor filtering, newest-first, eight-field safe DTO with verbatim metadata/hashes, read-only with no self-emission); verification stays an internal service with no HTTP exposure. No admin/pagination (Phase 13); see DECISIONS.md ADR-022/ADR-023.
...
- **Observability baseline & HTTP hardening (IMPLEMENTED, Phase 13-D).** Minimal Actuator (`health` + `info` only, details never shown, both authenticated, same port); explicit security headers with locked literals (HSTS secure-only); deny-by-default CORS for `/api/**` with exact origins; `OPTIONS /api/**` permitted for preflights while genuine preflights alone bypass rate limiting (four-condition rule). No metrics, tracing, probes, or management port. See DECISIONS.md ADR-027.
...
- **Observability:** minimal Actuator baseline is implemented (Phase 13-D: authenticated `health` + `info`, details never shown); metrics, tracing, probes, a separate management port, and structured-logging hardening remain future (Phase 13+).
- **Cloud deployment:** local containerization is implemented (Phase 13-E: multi-stage backend image, Compose stack with PostgreSQL, env-only secrets, authenticated-Actuator healthchecks; see DECISIONS.md ADR-028). External hosting, TLS termination, and scaling decisions remain FUTURE; no cloud dependency exists in the architecture.
...
| 8 | **Authentication & Security** | User entity; real security filter chain; JWT access/refresh; BCrypt; authorization; strict user-data isolation tests. *New dependency: JWT library (chosen in 8B).* Delivered incrementally: **8A IMPLEMENTED** (credential model V16 nullable `email`/`password_hash` + unique email, `BCryptPasswordEncoder`, `POST /api/v1/auth/register` → 201 `{id,email}`, 409 duplicates, minimal stateless chain with problem+json 401/403; no JWT/login/roles — see DECISIONS.md ADR-017); **8B IMPLEMENTED** (Nimbus JOSE+JWT HS256 access tokens, claims `sub`/`iat`/`exp`, `POST /api/v1/auth/login` → 200 `{accessToken,tokenType,expiresIn}`, uniform 401 on all credential failures, `AuthenticatedUser` principal + `requireUserId` web helper, Bearer filter before `UsernamePasswordAuthenticationFilter`; no V17 — V1–V16 unchanged; refresh deferred — see DECISIONS.md ADR-018); **8C FUTURE** (refresh tokens, only on demonstrated need). |
...
| 10 | **Notifications** | Notification entity; in-app feed with read/unread; emission at pipeline completion (10A, service layer), automatic single-owner fan-out from scheduled observations (10B-1, service layer), and authenticated REST feed (10B-2B); tests. |
