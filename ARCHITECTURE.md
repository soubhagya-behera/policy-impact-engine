...
- **JWT access/refresh tokens (PLANNED, Phase 8B/8C).** Short-lived access tokens (~15 min, Phase 8B) plus longer-lived refresh tokens (Phase 8C, only on demonstrated need); signing secrets supplied exclusively via environment configuration, never committed. No JWT in Phase 8A.
